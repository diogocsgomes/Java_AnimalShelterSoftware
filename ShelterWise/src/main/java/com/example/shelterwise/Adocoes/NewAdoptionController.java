package com.example.shelterwise.Adocoes;

import com.example.shelterwise.Modelos.Adopter;
...
public class NewAdoptionController {
    ...
    public void switchNewAdopter(ActionEvent actionEvent) throws IOException {
        Parent root = FXMLLoader.load(Objects.requireNonNull(getClass().getResource("/com/example/shelterwise/register-adopter-view.fxml")));
        stage = (Stage)((Node)actionEvent.getSource()).getScene().getWindow();
        scene = new Scene(root);
        stage.setScene(scene);
        stage.show();
